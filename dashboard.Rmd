---
title: "Predicting Pine Tree Infestation"
subtitle: "HGEN 612: Methods in Data Science · Sina Mahdiani"
output: 
...
if (!file.exists("data/pine_beetle_1993.xlsx")) {
  stop(
    "This R Markdown file is not standalone. Download and extract the complete ",
    "GitHub repository, then run render_dashboard.R.",
    call. = FALSE
  )
}
...
# Load the course-provided dataset.
data_work1 <- read_excel("data/pine_beetle_1993.xlsx", sheet = 1)
...
<div style="background:#f5f1e8; border-left:6px solid #5d7a32; padding:18px; border-radius:6px;">
Estimate the minimum linear distance to the nearest brood tree
(<code>DeadDist</code>) from tree-level and neighborhood-level measurements.
The dashboard compares regularized and conventional regression approaches and
summarizes model diagnostics for an academic data-science exercise.
The <code>tidymodels</code> workflows use <code>sqrt(DeadDist)</code> as their
outcome, so their RMSE values are reported on the square-root distance scale.
</div>
